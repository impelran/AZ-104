...
apt-get update -y
apt-get install -y python3 python3-pip python3.8-venv git gunicorn libpq-dev curl
...
set -ex
# Source environment variables for Django management commands
set -a
...
# The Django settings.py should be configured to use these environment variables
